import { z } from "zod";
import { eq, and, desc, asc, gte, lte, or, between, sql } from "drizzle-orm";
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!isAdmin) {
        ...
        if (accessRules.length > 0) {
          ...
          const hasAllow = accessRules.some(rule => rule.accessType === "allowed");
      ...
      if (resource.isIndivisible && input.requestedQuantity !== resource.totalCapacity) {
        throw new Error(`Resource is indivisible and requires full allocation of ${resource.totalCapacity} ${resource.capacityUnit}`);
      }

      if (resource.minAllocation && input.requestedQuantity < resource.minAllocation) {
        throw new Error(`Requested quantity is below minimum allocation of ${resource.minAllocation} ${resource.capacityUnit}`);
      }

      if (resource.maxAllocation && input.requestedQuantity > resource.maxAllocation) {
        throw new Error(`Requested quantity exceeds maximum allocation of ${resource.maxAllocation} ${resource.capacityUnit}`);
      }
      ...
      if (input.requestedQuantity > availableCapacity) {
        throw new Error(`Insufficient capacity. Available: ${availableCapacity} ${resource.capacityUnit}, Requested: ${input.requestedQuantity} ${resource.capacityUnit}`);
      }
      ...
      if (input.bookingType === "exclusive" && !resource.isIndivisible) {
        if (input.requestedQuantity !== resource.totalCapacity) {
          throw new Error("Exclusive bookings require full resource capacity allocation");
        }
      }
      ...
      // Create the booking
      const [newBooking] = await ctx.db.insert(bookings).values({
        ...input,
        userId: ctx.session.user.id,
        status: isAdmin ? "approved" : "pending", // Admin bookings auto-approved
        ...(isAdmin && { approvedById: ctx.session.user.id, approvedAt: new Date() }),
      }).returning();
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.startTime || input.endTime || input.requestedQuantity) {
        ...
        const newQuantity = input.requestedQuantity || booking.requestedQuantity;
        ...
        if (newQuantity > availableCapacity) {
          throw new Error(`Insufficient capacity. Available: ${availableCapacity} ${resource.capacityUnit}, Requested: ${newQuantity} ${resource.capacityUnit}`);
        }
      ...
      if (input.requestedQuantity) updateData.requestedQuantity = input.requestedQuantity;
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.allocatedQuantity) {
        if (input.allocatedQuantity > booking.requestedQuantity) {
          throw new Error("Allocated quantity cannot exceed requested quantity");
        }
    ...
    .query(async ({ ctx, input }) => {
      ...
      return calendarBookings.map(booking => ({
        ...
        color: booking.status === "pending" ? "#fbbf24" : 
               ...
               booking.status === "active" ? "#3b82f6" : "#6b7280",
        extendedProps: {
