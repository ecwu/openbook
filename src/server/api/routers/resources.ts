...
export const resourcesRouter = createTRPCRouter({
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (!isAdmin && input.onlyAccessible) {
        ...
        resourcesList = resourcesList.filter(resource => {
          ...
          const hasDeny = userAccessRules.some(rule => rule.accessType === "denied");
          if (hasDeny) {
            return false;
          }

          // Check for explicit allow
          const hasAllow = userAccessRules.some(rule => rule.accessType === "allowed");
          return hasAllow;
        });
      }
      ...
      return resourcesList.map(resource => {
        ...
        const currentUtilization = resource.bookings.reduce((total, booking) => {
        ...
        return {
          ...
          utilizationPercentage: (currentUtilization / resource.totalCapacity) * 100,
        };
      });
    }),
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [newResource] = await ctx.db.insert(resources).values(input).returning();
      return newResource;
    }),
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.specifications) updateData.specifications = input.specifications;
      ...
      if (input.isIndivisible !== undefined) updateData.isIndivisible = input.isIndivisible;
  ...
  checkAvailability: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (resource.isIndivisible && overlappingBookings.length > 0) {
        validations.push({
          type: "error", 
          message: "Indivisible resource is already allocated during this time period",
        });
      }
      ...
      const isAvailable = validations.filter(v => v.type === "error").length === 0;
