  ...
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return groupsList.map(group => ({
        ...
        accessibleResources: group.resourceAccess.filter(ra => ra.accessType === "allowed"),
        deniedResources: group.resourceAccess.filter(ra => ra.accessType === "denied"),
      }));
    }),
  ...
  getAccessibleResources: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const allowedResources = allResources.filter(resource => {
        ...
        const hasAllowRule = accessRules.some(rule => rule.accessType === "allowed");
        const hasDenyRule = accessRules.some(rule => rule.accessType === "denied");
